...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        $insert_query = "INSERT INTO uzytkownicy (imie, nazwisko, email, telefon, data_urodzenia, haslo, kategoria_prawa_jazdy) VALUES (?, ?, ?, ?, ?, ?, ?)";
        
        $stmt = $conn->prepare($insert_query);
        $stmt->bind_param("sssssss", $imie, $nazwisko, $email, $telefon, $data_urodzenia, $hashed_password, $kategoria_prawa_jazdy);
    ...
    if (!empty($errors)) {
        ...
        header("Location: register.html?errors=" . $error_string);
        exit();
    }
}
